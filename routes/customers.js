    ...
    router.get('/datatable', isLoggedIn, async (req, res) => {
      ...
      router.get('/edit/:customerid', isLoggedIn, async function (req, res, next) {
        try {
          ...
          const getEdit = await db.query("SELECT * FROM customers WHERE customerid = $1", [id])
      ...
      router.post('/edit/:customerid', isLoggedIn,async function (req, res, next) {
        try {
          const id = req.params.customerid
          const { name, address, phone } = req.body
          await db.query("UPDATE customers SET name = $1, address = $2, phone = $3 WHERE customerid = $4",[name, address, phone, id])
          res.redirect('/customers')
        } catch (error) {
          console.log(error);
          res.send(error)
