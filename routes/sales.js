...
module.exports = function (db) {
    ...
    router.get('/create', isLoggedIn, async function (req, res, next) {
        try {
            ...
            const inputSales = await db.query('INSERT INTO sales(totalsum, customer, operator) VALUES(0, $1, $2) returning *', [userid, customer])
            res.redirect(`/sales/show/${inputSales.rows[0].invoice}`)
        } catch (error) {
            console.log(error);
            res.jsonp(error)
        }
    ...
    router.get('/show/:invoice', isLoggedIn, async function (req, res, next) {
        try {
            const getInv = await db.query('SELECT sales.*, customers.* FROM sales LEFT JOIN customers ON sales.customer = customers.customerid WHERE invoice = $1', [req.params.invoice])
            const getAdd = await db.query('SELECT * FROM goods ORDER BY barcode')
            const getCus = await db.query('SELECT * FROM customers ORDER BY customerid')
    ...
    router.post('/additem', isLoggedIn, async function (req, res, next) {
        try {
            ...
            const data = await db.query('SELECT * FROM sales WHERE invoice = $1', [invoice])
